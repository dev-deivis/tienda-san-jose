...
import { MapPin, Plus, Star, Pencil, Trash2, X, CheckCircle } from 'lucide-react';
...
function validateForm(data: AddressFormData): FormErrors {
  ...
  if (!data.direccion1.trim()) errs.direccion1 = 'La dirección es obligatoria';
  if (!data.ciudad.trim()) errs.ciudad = 'La ciudad es obligatoria';
  if (!data.estado) errs.estado = 'El estado es obligatorio';
...
export default function DireccionesPage({
  ...
  const fetchAddresses = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setFetchError('Error al cargar las direcciones.');
        return;
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setSaveError('Error al guardar la dirección. Intenta de nuevo.');
        return;
      }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        setDeleteError('Error al eliminar la dirección. Intenta de nuevo.');
        return;
      }
      setDeletingId(null);
      await fetchAddresses();
  ...
  const setDefault = async (id: number) => {
    ...
    try {
      ...
      if (!res.ok) {
        setDefaultError('Error al actualizar. Intenta de nuevo.');
        return;
      }
      await fetchAddresses();
    } catch {
      setDefaultError('Error de conexión. Intenta de nuevo.');
    } finally {
      ...
      <div className="max-w-2xl mx-auto">
        ...
        {fetchError ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-200 py-16 text-center">
            ...
            <p className="text-sm font-medium text-gray-600">No tienes direcciones guardadas.</p>
              ...
              >
                ...
                {addr.isDefault && (
                  <div className="flex items-center gap-1 mb-3">
                    ...
                    <span className="text-xs font-semibold text-brand-purple">Predeterminada</span>
                  </div>
                )}
                ...
                <div className="flex items-center gap-3 mt-4 flex-wrap">
                  {!addr.isDefault && (
                    ...
                    >
                      <Star size={13} />
                      {settingDefaultId === addr.id ? 'Actualizando…' : 'Establecer como predeterminada'}
                    </button>
                  )}
                  <button
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg my-auto">
            ...
            <div className="px-6 py-5 space-y-4 max-h-[70vh] overflow-y-auto">
              ...
              <label className="flex items-center gap-2.5 cursor-pointer select-none">
                ...
                />
                <span className="text-sm text-gray-700">Establecer como predeterminada</span>
              </label>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
            ...
            <div className="flex gap-3">
              ...
              >
                {confirmingDelete ? 'Eliminando…' : 'Sí, eliminar'}
              </button>
            </div>
          </div>
        </div>
